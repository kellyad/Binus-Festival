...
var subView = {
    ...
    onLoaded: function(arg)
    {
        ...
        $('#btnGenerate').click(function () {
                ...
                //     });
            BM.ajax({
            url : BM.serviceUri + "BifestController/SystemHandler/generateSchedule",
            type: "POST",
    ...
    publishData : function() {
        var sx = this;
        BM.ajax({
            url : BM.serviceUri + "BifestController/SystemHandler/publishSchedule",
            type: "POST",
            data: JSON.stringify({ Period: $('#ddlPeriodPopUp option:selected').val() , SystemID: $('#ddlSystemPopUp option:selected').val() }),
    ...
    unpublishData : function() {
        var sx = this;
       BM.ajax({
            url : BM.serviceUri + "BifestController/SystemHandler/unpublishSchedule",
            type: "POST",
            data: JSON.stringify({ Period: $('#ddlPeriodPopUp option:selected').val() , SystemID: $('#ddlSystemPopUp option:selected').val() }),
    ...
    loadInit: function() {
        ...
        var sx = this;
        BM.ajax({
            url : BM.serviceUri + "BifestController/SystemHandler/getAllGenerateData",
            type: "POST",
            data: JSON.stringify({ Period: $('#ddlPeriodPopUp option:selected').val() , SystemID: $('#ddlSystemPopUp option:selected').val() }),
            beforeSend: function(){
         ...
         $('#tableOfficialNotesPopUp tbody').append('<tr class="dataPopUpLoading"><td colspan="24" style="text-align:center;"><div id="iLoading"/></td></tr>');
            ...
            success: function(data){
                ...
                $('#tableOfficialNotesPopUp tbody').append('<tr class="dataAvailable"><td colspan="24" style="text-align:center;">data was not available</td></tr>');
                    ...
                    var c = $('#iTemplateOfficialNotesPopUp').clone().removeAttr('id').css('display', '').addClass('dataPopUp');
                    $('.iOption', c).append('<a target="_blank" href="'+BM.baseUri+"newstaff/#/event/binusfestival/SystemConfiguration."+e.SystemID+'"class="icon icon-edit"  style="cursor:pointer;"></a>');
                    // var d = $('#formparticipant .custom-combobox').clone();
    ...
    ExportToExcel : function() {
        ...
        var data = {};
            ...
            form.action = BM.serviceUri+'BifestController/SystemHandler/getExportData';
                form.method = 'POST';
                form.target = '_blank';
                if (data) {
                    for (var key in data) {
