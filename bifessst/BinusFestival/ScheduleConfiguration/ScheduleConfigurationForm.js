...
var popupSubView = {
    ...
    onLoaded: function(arg) {
        ...
        BM.ajax({
                ...
                $('#loaderEventType').hide();
                    ...
                    if(data.data.length>0) {
                        for(i=0 ; i < data.data.length ; i++){
                            option = "<option value='"+data.data[i].EventTypeId+"'>"+data.data[i].EventTypeName+"</option>";
         ...
         // if(typeof BM.filter.ScheduleConfigurationID!="undefined"){
         //    console.log("masuk");
         // }
                                            ...
                                            if ($('.txtDuration').val()>0) {
                                                if ($('.txtPostDuration').val()>=0) {
                                                    BM.ajax({
                                                        ...
                                                        dataType: "json",
                                                        beforeSend: function() {
                                                           $('.dataPreviewPopUp').remove();
                                                            $('.iLoadingPopup').html($('<img alt="" />').attr('src', BM.baseUri + 'staff/images/loading.gif'));
                                                            $('#loadingpopup').show();
                                                        },
                                                        success: function(result) {
                                                        $("#actionTopic").html($('.txtTopic').val());
                                                        $("#actionStartDate").html(new Date($('#TxtStartDate').val()).toString("dd-MMMM-yyyy"));
                                                        $("#actionEndDate").html(new Date($('#TxtEndDate').val()).toString("dd-MMMM-yyyy"));
                                                        $("#PopupMessage").show();
                                                                $('#BeforePrev').hide();
                                                        $.each(result, function(a, e) {
                                                                var c = $('#iTemplatePopUp').clone().removeAttr('id').css('display', '').addClass('dataPreviewPopUp');
                                                    ...
                                                    //  BM.filter={Topic : $('.txtTopic').val(), StartDate : $('#TxtStartDate').val(), EndDate : $('#TxtEndDate').val(), 
                                                    //  StartTime : start, EndTime : end,  ShiftDuration: $('.txtDuration').val(),
                                                    //                  PostDuration: $('.txtPostDuration').val() };
                                                    // window.location = "#/event/binusfestival/konfigurasiJadwalRuangan."+BM.filter.SystemID+"#Preview";
                                                    //         //console.log($('#ddlEvent').val() + ' ' + $('#ddlRoomUsage').val()); return;
                                                            // $(this).attr('disabled', 'disabled');
                                                            // BM.ajax({
