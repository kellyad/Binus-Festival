...
var subView = {
    ...
    onLoaded: function(arg)
    {
      ...
      //  $('#lblSession').html(this.subject);
        ...
        BM.filter={SystemID : arg, ParticipantGroupID: sx.subject.group[sx.subject.current].ParticipantGroupID,SessionID:sx.subject.session+1};
                        window.location = "#/event/binusfestival/ScheduleConfiguration."+arg+"#ScheduleConfigurationForm";
                    }); 
        ...
        BM.filter={SystemID : arg, ParticipantGroupID: sx.subject.group[sx.subject.current].ParticipantGroupID,ParticipantGroupName: sx.subject.group[sx.subject.current].ParticipantGroupName,SessionID:sx.subject.session+1};
                        window.location = "#/event/binusfestival/ScheduleConfiguration."+arg+"#GroupingConfiguration";
                    });
       ...
       // BM.filter={SystemID : arg, ParticipantGroupID: sx.subject.group[sx.subject.current].ParticipantGroupID,SessionID:sx.subject.current};
        if($('#btnSubmit').val()=="Save And Finish") {
            window.location = "#/event/binusfestival/SystemConfiguration";
        }else{
         if(!hasSubmitted){
            sx.subject.session++;
    ...
    loadTable: function() {
        ...
        BM.ajax({
            url : BM.serviceUri + "BifestController/ScheduleHandler/getTotalParticipantByParticipantGroupID",
            type: "POST",
            beforeSend: function() {
                $('.iLoadingPopup').html($('<img alt="" />').attr('src', BM.baseUri + 'staff/images/loading.gif'));
         ...
         BM.ajax({
            url: BM.serviceUri + "BifestController/ScheduleHandler/getScheduleConfiguration",
            ...
            success: function(result) {
                ...
                $.each(result, function(a, e) {
                    ...
                    BM.filter={SystemID : parameter, ParticipantGroupID : e.ParticipantGroupID, ScheduleConfigurationID : e.ScheduleConfigurationID, SessionID : e.Session};
                        window.location = "#/event/binusfestival/ScheduleConfiguration."+parameter+"#ScheduleConfigurationForm";
                    });
                    $('.iOption .icon-trash', c).click(function(event){
                        BM.ajax({
                                url: BM.serviceUri + "BifestController/ScheduleHandler/deleteScheduleConfiguration",
    ...
    ExportToExcel : function() {
        ...
        var data = {};
            ...
            form.action = BM.serviceUri+'BifestController/ScheduleHandler/getExportData';
                form.method = 'POST';
                form.target = '_blank';
                if (data) {
                    for (var key in data) {
